...
import LedControl from "./LedControl"; // Import the LedControl component
...
const DeviceCard = ({ icon: Icon, name, description, token }) => {
  ...
  const getTemperatureColor = () => {
    if (temperature < 26) return "bg-blue-400";
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-lg shadow-lg text-sm md:text-base text-white relative">
      ...
      {name === "Fan" && <FanControl speed={speed} setSpeed={setSpeed} isOn={isOn} />}
      {name === "Temperature" && <TemperatureControl temperature={temperature} renderScale={renderScale} getTemperatureColor={getTemperatureColor} />}
      {name === "Humidity" && <HumidityControl humidity={humidity} renderScale={renderScale} getHumidityColor={getHumidityColor} />}
      {name === "LED" && <LedControl brightness={brightness} setBrightness={setBrightness} isOn={isOn} autoMode={autoMode} setAutoMode={setAutoMode} token={token} />}
    </div>
  );
};
